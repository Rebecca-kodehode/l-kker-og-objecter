...
// random number generator for the length of cities
...
for (let i = 0; i <= people.length - 1; i++) {
  ...
  people[i].city = cities[randomNumber];

  const title = people[i].male ? "Mr" : "Ms";

  people[i].name = title + " " + people[i].name;
  // people[i].name = `${title} ${people[i].name}`;
...
// prøv unshift("coding") for siste del
...
function rollDice(diceroll) {
  // const variabel som refererer til arrayet du skal returnere (tomt array i starten)
  const result = [];
  for (let i = 0; i < diceroll; i++) {
    // random number generator (1-6)
    let randomNumber = Math.ceil(Math.random() * diceroll);
    // push random number inn i variabelen fra starten
...
function fullQuote(quoteArray) {
  const cleanedWords = [];
  for (let word of quoteArray) {
    const eachWord = word.trim().toLowerCase();

    // console.log(eachWord);
    cleanedWords.push(eachWord);
  }
